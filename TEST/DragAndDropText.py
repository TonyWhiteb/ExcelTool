...
DROP_TARGET = wx.NewId()

class TextCtrl(FileCtrl.FileCtrl):
...
class TextDropTarget(wx.TextDropTarget):
   ...
   def OnDropText(self, x, y, data):
    ...
    #   self.obj.WriteText(data + '\n\n')
      self.obj.InsertStringItem(0,data)
...
class MainWindow(wx.Frame):
   ...
   def __init__(self,parent,id,title):
    ...
    #    self.text = wx.TextCtrl(self, DRAG_SOURCE, "", pos=(10,15), size=(350,500), style = wx.TE_MULTILINE|wx.HSCROLL)
       self.text = TextCtrl(self,DRAG_SOURCE,pos=(10,15), size=(350,500), style = wx.LC_REPORT|wx.BORDER_SUNKEN)
    ...
    #       self.text.WriteText("This is line %d of some text to drag.\n" % x)
       self.text.InsertColumn(0,'Test')

       for x in range(20):
           self.text.InsertStringItem(x,"This is line %d of some text to drag." % x)
       ...
       wx.EVT_LEFT_DOWN(self.text, self.OnDragInit)

       # Define a Text Control to recieve Dropped Text
       # Label the control
       wx.StaticText(self, -1, "Text Drop Target", (370, 1))
       # Create a read-only Text Control
       self.text2 = TextCtrl(self, DROP_TARGET, pos=(370,15), size=(410,235), style = wx.LC_REPORT|wx.BORDER_SUNKEN)
       # Make this control a Text Drop Target
       # Create a Text Drop Target object
   ...
   def OnDragInit(self, event):
       ...
       text = self.text.GetItemText(self.text.GetId())
       tobj = wx.PyTextDataObject(text)
       src = wx.DropSource(self.text)
       src.SetData(tobj)
       src.DoDragDrop(True)
       self.text.DeleteItem(self.text.GetId())
